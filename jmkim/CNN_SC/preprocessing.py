...
# train 데이터 path
train_data = Path.cwd() / 'NLP_Tensorflow2' / 'nsmc-master' / 'ratings_train.txt'
...
# train 데이터 tab으로 저장
train.to_csv(Path.cwd() / 'NLP_Tensorflow2' / 'CNN_SC' / 'new' / 'data_in' / 'train.txt', sep='\t', index=False)
# valid 데이터 tab으로 저장
eval.to_csv(Path.cwd() / 'NLP_Tensorflow2' / 'CNN_SC' / 'new' / 'data_in' / 'val.txt', sep='\t', index=False)

#test 데이터 path
test_data = Path.cwd() / 'NLP_Tensorflow2' / 'nsmc-master' / 'ratings_test.txt'
# test data load (document, label 컬럼을 tab으로 구별)
data = pd.read_csv(test_data, sep='\t').loc[:, ['document', 'label']]
# test data에서 Nan 값 지우고
data = data.loc[data['document'].isna().apply(lambda elm: not elm), :]
# test 데이터 tab으로 저장
data.to_csv(Path.cwd() / 'NLP_Tensorflow2' / 'CNN_SC' / 'new' / 'data_in' / 'train.txt', sep='\t', index=False)